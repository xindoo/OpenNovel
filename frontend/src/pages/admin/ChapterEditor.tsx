...
export function ChapterEditor({ novelId, onSwitchToUpload }: ChapterEditorProps) {
  ...
  const loadChapters = useCallback(async (id: number) => {
  ...
  const loadChapterContent = useCallback(async (id: number, chapterId: number) => {
    setLoadingContent(true);
    setError('');
    setChapterTitle('');
    setChapterContent('');

    const result = await getChapter(id, chapterId);
    if (result.success && result.data) {
  ...
  const handleChapterClick = (chapter: Chapter) => {
    ...
    loadChapterContent(novelId, chapter.id);
  };
  ...
  const handleSave = async () => {
    ...
    const result = await updateChapter(novelId, selectedChapterId, {
      title: chapterTitle,
  ...
  const handleDelete = async () => {
    ...
    const result = await deleteChapter(novelId, selectedChapterId);
